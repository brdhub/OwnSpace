...
Describe "OwnSpace update validation" {
  It "only accepts a strictly newer semantic version" {
    ...
    { ConvertTo-OwnSpaceVersion "release-latest" } | Should Throw "Invalid OwnSpace version:"
  }
  ...
  It "rejects a manifest with an extra field or wrong archive name" {
    ...
    { Read-OwnSpaceUpdateManifest -Path $extraPath } | Should Throw "OwnSpace update manifest has an unexpected schema."
    { Read-OwnSpaceUpdateManifest -Path $wrongPath } | Should Throw "OwnSpace update archive name does not match its version."
  }

  It "rejects protected and traversal archive entries" {
    { Test-OwnSpaceArchiveEntry -EntryName "../data/ownspace.db" } | Should Throw "OwnSpace update archive contains a path outside the application directory."
    { Test-OwnSpaceArchiveEntry -EntryName "data/ownspace.db" } | Should Throw "OwnSpace update archive contains protected path: data"
    { Test-OwnSpaceArchiveEntry -EntryName "OwnSpace/.env.local" } | Should Throw "OwnSpace update archive contains protected path: .env.local"
    Test-OwnSpaceArchiveEntry -EntryName "OwnSpace/src/app/page.tsx" | Should Be $true
  }
  ...
  It "keeps resolved child paths inside the intended root" {
    ...
    { Assert-OwnSpaceChildPath -Root $root -Path (Join-Path $root "../outside.txt") } | Should Throw "Path leaves the allowed root:"
  }
  ...
  It "rejects prereleases and assets outside the fixed GitHub repository" {
    ...
    { Select-OwnSpaceReleaseAssets -Release $release } | Should Throw "OwnSpace only installs final GitHub Releases."
    ...
    { Select-OwnSpaceReleaseAssets -Release $release } | Should Throw "OwnSpace release asset is outside the trusted repository."
  }
  ...
  It "allows only HTTPS GitHub asset redirect hosts" {
    ...
    { Test-OwnSpaceAllowedDownloadUri -Uri ([uri]"http://github.com/file") } | Should Throw "OwnSpace update download was redirected outside trusted GitHub hosts."
    { Test-OwnSpaceAllowedDownloadUri -Uri ([uri]"https://evil.example/file") } | Should Throw "OwnSpace update download was redirected outside trusted GitHub hosts."
  }
